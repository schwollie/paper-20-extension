...
class CustomLogger(Logger):
    ...
    def regression(self):
        ...
        return {
            ...
            'spearmanr': reformat(eval_spearmanr(torch.squeeze(true).numpy(),
                                                 torch.squeeze(pred).numpy())['spearmanr']),
            'pearson': reformat(eval_pearson(torch.squeeze(true).numpy(), torch.squeeze(pred).numpy())),
...
def eval_pearson(y_true, y_pred):
    try:
        pearsonr_rho, _ = pearsonr(y_true, y_pred)
        return pearsonr_rho
    except:
        return -1