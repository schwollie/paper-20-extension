...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in tqdm(enumerate(loader), desc='train'):
        batch.split = 'train'
        batch.to(torch.device(cfg.device))
        pred, true = model(batch)
        trivial_pred = torch.mean(true) * torch.ones_like(pred)
        if cfg.dataset.name == 'ogbg-code2':
            loss, pred_score = subtoken_cross_entropy(pred, true)
            trivial_loss, _ = subtoken_cross_entropy(trivial_pred, true)
            _true = true
            _pred = pred_score
        else:
            loss, pred_score = compute_loss(pred, true)
            trivial_loss, _ = compute_loss(trivial_pred, true)
