...
from graphgps.layer.gatedgcn_layer import GatedGCNLayer, HeteroGatedGCNGraphGymLayer, HeteroGatedGCNLayer
...
class CustomGNN(torch.nn.Module):
    ...
    def build_conv_model(self, model_type):
        if model_type == 'gatedgcnconv':
            ...
            return MLPLayer
        elif model_type == 'hetero_gatedgcnconv':
            return HeteroGatedGCNLayer
        else:
            raise ValueError("Model {} unavailable".format(model_type))

    def forward(self, batch):
        for module in self.children():
            if self.model_type in ['gcniiconv']:
