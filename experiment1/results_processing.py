...
def find_best_stat(stats_results):
    ...
    for stat in stats_results:
        ...
        with_mvn10 = stat['folder_parts'][0] == 'MVN10'
        with_hsg = stat['folder_parts'][0] == 'HSG'
        ...
        best_stats.append({'model': model_name, 'params': params, 'measure': measurement, 'vn': with_vn, 'mvn5': with_mvn5, 'mvn10': with_mvn10, 'hsg': with_hsg,'dataset': data_set_name, 'stat': best_stat})

    return best_stats


def generate_table(data_set_name, stats, with_extension) -> pd.DataFrame:
    ...
    mvn10_data = [d for d in data if d['mvn10']]
    hsg_data = [d for d in data if d['hsg']]
    no_vn_data = [d for d in data if (not d['vn'] and not d['mvn5'] and not d['mvn10'] and not d['hsg'])]
    ...
    mvn10_stats = [(d['model'], d['stat']) for d in mvn10_data]
    hsg_stats = [(d['model'], d['stat']) for d in hsg_data]
    ...
    mvn10_stats = sorted(mvn10_stats, key=lambda x: x[0])
    hsg_stats = sorted(hsg_stats, key=lambda x: x[0])
    ...
    if with_extension:
        return pd.DataFrame({
            ...
            measure + ' (after 5 VN)': [x[1] for x in mvn5_stats] + [None] * (len(no_vn_stats) - len(mvn5_stats)),
            measure + ' (after 10 VN)': [x[1] for x in mvn10_stats] + [None] * (len(no_vn_stats) - len(mvn10_stats)),
            measure + ' (after HSG)': [x[1] for x in hsg_stats] + [None] * (len(no_vn_stats) - len(hsg_stats))})
